...
function saveCache($name = '',$id='') {
	...
	/*系统配置*/
		...
		//delete groupid.when update menu data the listorder is disorder
		$list = $Model->order('seq,groupid')->select ();
		//$list = $Model->order('groupid,seq')->select ();
		//modified by zhangys 2013.04.19 end
		$pkid = $Model->getPk ();
		$data = array ();
		foreach ( $list as $key => $val ) {
			$data [$val [$pkid]] = $val;
		...
		F($name,$data);
	}else if ($name == "teach_task"){
		$list = $Model->order('teachid')->select ();
		$pkid = $Model->getPk ();
		$data = array ();
		foreach ( $list as $key => $val ) {
			$data [$val [$pkid]] = $val;
		...
		$list = $Model->order('courseid')->select ();
		$pkid = $Model->getPk ();
		$data = array ();
		foreach ( $list as $key => $val ) {
			$data [$val [$pkid]] = $val;
		...
		F($name,$data);
	}else if ($name == "dd_list"){
		/*add by zhangys 2013-04-26 将数据字典放入缓存*/
		$Model = new DataDictModel();
		$data_dict_list = $Model
						->field("enum_name,fd_value")
						->order("enum_name")
						->select();

		$data_dict_cache = Array();

		foreach ($data_dict_list as $data_dict) {
			$enum_name = $data_dict["enum_name"];
			$fd_value = $data_dict["fd_value"];
			$data_dict_cache[$enum_name] = $fd_value;
		}
		F($name,$data_dict_cache);
	}else{
 ...
 */
function log_data($array){
	$fp = fopen(C(TEMP_PATH)."info.txt",'ab'); //以二进制追加方式打开文件,没文件就创建
...
/**
 * 将php数组转换成json然后当作js变量向模板输出
 */
function define_json($js_name,$arr) {
	$data = json_encode($arr);
	echo "<script>";
	echo "var {$js_name}={$data}";
	echo "</script>";
}
 ...
 * 得到教学工作量的比重，在排课管理中使用
 */
function get_teach_workload_percent($workload) {
	$max = 10 * 5 * 18;	//每天10个学时，每周5天，共18周=900

	$percent = $workload / $max * 100;
 ...
 * 列出班级信息
 */
function list_class($classes){
	$index_limit = count($classes);

	$one_line_num = 3;	//每行显示的个数

	for($index = 0;$index < $index_limit;$index ++){
		if($index % $one_line_num == 0){
			echo "<tr>";
		}
		echo '<td width="25%">';
		echo "<label for='class".$classes[$index]['classid']."' class='noselItem'>";
		echo "<input type='checkbox' name='courseids[]'";
		echo        "id='class".$classes[$index]['classid']."' value='".$classes[$index]['majorid']."'>";
		echo $classes[$index]['name'];
		echo "</label>";
		echo "</td>";
		if($index % $one_line_num == $one_line_num - 1){
			echo "</tr>";
		}
	}
	if($index % $one_line_num != 0){
